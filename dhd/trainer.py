...
from dhd.config import get_cfg
from dhd.losses import SegmentationLoss, SpatialRegressionLoss
from dhd.metrics import IntersectionOverUnion, PanopticMetric, BandWidth
from dhd.models.dhd import DHD
from dhd.utils.instance import predict_instance_segmentation
from dhd.utils.visualisation import visualise_output
...
class TrainingModule(pl.LightningModule):
    ...
    def shared_step(self, batch, is_train):
        ...
        # Forward pass
        output = self.model(image, intrinsics, extrinsics, future_egomotion, future_distribution_inputs, is_train=is_train)
        ...
        if not is_train:
            ...
            # Calculate metrics
            if self.cfg.INSTANCE_FLOW.ENABLED:
                self.metric_iou_val(torch.argmax(output['segmentation'].detach(), dim=2, keepdims=True)[:, 1:],
                                    labels['segmentation'][:, 1:])
                self.metric_panoptic_val(pred_consistent_instance_seg[:, 1:], labels['instance'][:, 1:])
            else:
                self.metric_iou_val(torch.argmax(output['segmentation'].detach(), dim=2, keepdims=True),
                                    labels['segmentation'])
    ...
    def prepare_future_labels(self, batch):
        ...
        label_time_range = self.model.receptive_field - 2
    ...
    def shared_epoch_end(self, step_outputs, is_train):
        ...
        self.logger.experiment.add_scalar('weights/segmentation_weight',
